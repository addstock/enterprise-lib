 ...
 * EnterpriseLib.array_mapper_path subkey within the configuration that points
...
class ArrayMapperFactory
{
    public function __invoke($services)
    {
        if (! $services->has('config')) {
            throw new DomainException('Cannot create EnterpriseLib\ArrayMapper; missing config dependency');
        }

        $config = $services->get('config');
        if (! isset($config['EnterpriseLib']['array_mapper_path'])) {
            throw new DomainException(sprintf(
                'Cannot create %s; missing EnterpriseLib.array_mapper_path configuration',
                ArrayMapper::class
            ));
        }

        $path = $config['EnterpriseLib']['array_mapper_path'];
        if (! file_exists($path)) {
            throw new DomainException(sprintf(
